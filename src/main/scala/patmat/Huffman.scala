...
object Huffman {
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs.sortBy(_._2).map(t => Leaf(t._1,t._2))
   ...
   */
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] =  bits match {
        case head::tail  => {
                        def readUntilLeaf(toBeRead: CodeTree, remainingBits: List[Bit]): (Char, List[Bit]) = toBeRead match {
                          case Leaf(c, _) => (c, remainingBits)
                          case Fork(left, _, _, _)  if (remainingBits.head == 0) => readUntilLeaf(left, remainingBits.tail)
                          case Fork(_, right, _, _)  if (remainingBits.head == 1) => readUntilLeaf(right, remainingBits.tail)
                        }
                        val (readChar, remainingBits) = readUntilLeaf(tree, head::tail)
                        readChar :: decode(tree, remainingBits)
                      }
        case Nil => Nil
      }
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = text match {
      case head::tail =>
                        ...
                        encodeChar(tree, head):::encode(tree)(tail)
      case Nil => Nil
    }
   ...
   */
    def codeBits(table: CodeTable)(char: Char): List[Bit] = table match {
      case head :: _ if head._1 == char => head._2
   ...
   */
    def convert(tree: CodeTree): CodeTable = tree match {
      case Fork(left, right, _, _) => convert(left).map(t => (t._1, 0::t._2)):::((convert(right).map(t => (t._1, 1::t._2))))
      case _:Leaf => Nil
    }
